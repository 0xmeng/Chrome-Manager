...
from proxy_setting_view import FolderFileTable
...
import random
from clash_proxy import ClashAPI
...
class ChromeManager:
    def __init__(self):
        ...
        self.clash_dir = self.settings.get('clash_dir', '')
    ...
    def create_widgets(self):
        ...
        ttk.Button(first_row, text="导入窗口", command=self.import_windows, style='Accent.TButton').pack(side=tk.LEFT,
                                                                                                         padx=2)
                        ...
                        try:
                            ...
                            if is_master:
                                ...
                                if is_floating and target_popups:
                                    ...
                                    min_diff = float('inf')
                                    current_size = (
                                    current_rect[2] - current_rect[0], current_rect[3] - current_rect[1])
                                    ...
                                    for popup in target_popups:
                                        ...
                                        size_diff = abs(current_size[0] - popup_size[0]) + abs(
                                            current_size[1] - popup_size[1])
                            ...
                            if isinstance(event, mouse.WheelEvent):
                                try:
                                    ...
                                    if keyboard.is_pressed('ctrl'):
                                        if wheel_delta > 0:
                                            win32gui.PostMessage(target_hwnd, win32con.WM_KEYDOWN, win32con.VK_CONTROL,
                                                                 0)
                                            win32gui.PostMessage(target_hwnd, win32con.WM_KEYDOWN, 0xBB,
                                                                 0)  # VK_OEM_PLUS
                                            ...
                                            win32gui.PostMessage(target_hwnd, win32con.WM_KEYDOWN, win32con.VK_CONTROL,
                                                                 0)
                                            win32gui.PostMessage(target_hwnd, win32con.WM_KEYDOWN, 0xBD,
                                                                 0)  # VK_OEM_MINUS
                            ...
                            # 处理鼠标点击
                                ...
                                if event.event_type == mouse.DOWN:
                                    if event.button == mouse.LEFT:
                                        ...
                                        win32gui.PostMessage(target_hwnd, win32con.WM_RBUTTONDOWN, win32con.MK_RBUTTON,
                                                             lparam)
                                    elif event.button == mouse.MIDDLE:  # 添加中键支持
                                        win32gui.PostMessage(target_hwnd, win32con.WM_MBUTTONDOWN, win32con.MK_MBUTTON,
                                                             lparam)
            ...
            if self.is_sync:
                ...
                if is_master or is_popup:
                    ...
                    if not hasattr(self, 'last_key_time') or current_time - self.last_key_time > 0.01:
                        ...
                        if hasattr(self,
                                   'last_key') and self.last_key == event.name and event.event_type == keyboard.KEY_DOWN:
                            return
                    ...
                    for hwnd in self.sync_windows:
                        try:
                            ...
                            modifiers = 0
                            modifier_keys = {
                                'ctrl': {'pressed': keyboard.is_pressed('ctrl'), 'vk': win32con.VK_CONTROL,
                                         'flag': win32con.MOD_CONTROL},
                                'alt': {'pressed': keyboard.is_pressed('alt'), 'vk': win32con.VK_MENU,
                                        'flag': win32con.MOD_ALT},
                                'shift': {'pressed': keyboard.is_pressed('shift'), 'vk': win32con.VK_SHIFT,
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.settings['clash_dir'] = self.clash_dir
            self.settings['proxies'] = self.proxies
    ...
    def set_proxies(self):
        ...
        if not ClashAPI.is_clash_config_exists():
            messagebox.showwarning("警告", "clash 配置文件未加载成功，请在设置中设置clash配置文件的目录后重试！")
            return
        ...
        proxy_dialog.title("配置代理")
        proxy_dialog.geometry("700x360")  # 增加窗口高度
        proxy_dialog.resizable(False, False)
        proxy_dialog.transient(self.root)
        proxy_dialog.grab_set()

        main_frame = ttk.Frame(proxy_dialog)
        ...
        try:
            icon_path = os.path.join(os.path.dirname(__file__), "app.ico")
            if os.path.exists(icon_path):
                proxy_dialog.iconbitmap(icon_path)
        except Exception as e:
            print(f"设置图标失败: {str(e)}")

        folders, files = ClashAPI.get_proxy_for_table()
        print(folders)
        print(files)

        # 创建表格组件并加入界面
        table = FolderFileTable(main_frame, folders, files, row_datas=window_numbers, selected_dict=self.proxies)
        ...
        def show_data():
            ...
            print(">>>>>> sorted_dict ")
            if sorted_dict:
                self.proxies.update(sorted_dict)
                print(self.proxies)
                self.save_settings()
        ...
        ttk.Button(main_frame, text="保存", command=show_data).grid(row=1, column=0, pady=10)

        # 居中显示
        self.center_window(proxy_dialog)
        self.root.wait_window(proxy_dialog)

    def check_browser_closed(self, proc, idx):
        if proc.poll() is not None:
            print(f"✅ 浏览器已关闭 编号：{idx}")
            clash = self.clash_servers.get(str(idx))
            if clash:
                clash.close_server()
                del self.clash_servers[str(idx)]
        else:
            self.root.after(1000, self.check_browser_closed, proc, idx)  # 每秒检查一次
    ...
    def open_windows(self):
        ...
        try:
            ...
            for num in window_numbers:
                ...
                # 不启用CDP，直接打开
                if self.clash_servers.get(str(num)):
                    clash = self.clash_servers.get(str(num))
                else:
                    clash = ClashAPI(num)
                    self.clash_servers[str(num)] = clash

                proxy = self.proxies.get(str(num))
                print(f"---->>> cl {num}")
                success = clash.switch_clash_proxy(proxy.get("proxy_group"), proxy.get("proxy_name"))
                if not success:
                    messagebox.showerror("错误", f"窗口{num} 代理启动失败")
                    continue
                ...
                print(f">>>> shortcut_obj target {target}")
                args = shortcut_obj.Arguments
                print(f">>>> shortcut_obj args {args}")
                # 拆分参数为列表（避免空格/引号问题）
                args_list = shlex.split(args)
                print(f">>>> arglist {args_list}")
                # 拼接最终命令
                command = [target] + args_list
                print(f">>>> command {command}")

                # launch_options = [
                #     # f'--remote-debugging-port={80000+num}',
                #     f'--user-data-dir={user_data_dir}',
                #     f'--proxy-server={clash.proxy}',
                #     # f'--load-extension={extension_path}',
                #     # f"--user-agent={acct.get('ua')}"
                # ]
                # if self.enable_cdp:
                #     launch_options = [
                #         f'--remote-debugging-port={9222 + num}',
                #         f'--user-data-dir={user_data_dir}',
                #         f'--proxy-server={clash.proxy}',
                #         # f'--load-extension={extension_path}',
                #         # f"--user-agent={acct.get('ua')}"
                ...
                # # 使用列表形式调用Popen，并传递chrome路径和启动参数
                # chrome_process = subprocess.Popen([chrome_path] + launch_options)

                def replace_or_add_arg(args_list, key, new_value, add_if_missing=True):
                    prefix = f'--{key}='
                    replaced = False

                    for i, arg in enumerate(args_list):
                        if arg.startswith(prefix):
                            args_list[i] = f'{prefix}{new_value}'
                            replaced = True
                            break

                    if not replaced and add_if_missing:
                        args_list.append(f'{prefix}{new_value}')
                ...
                if self.enable_cdp:
                    ...
                    command = replace_or_add_arg(command, "remote-debugging-port", debug_port)
                command = replace_or_add_arg(command, "proxy-server", clash.api_proxy)
                # 设置调试端口参数
                ...
                try:
                    ...
                    time.sleep(2)
                    self.check_browser_closed(proc, num)
                except Exception as e:
                ...
                try:
                                            ...
                                            if shortcut_file.endswith('.lnk'):
                                                ...
                                                try:
                                                    ...
                                                    if '--user-data-dir=' in shortcut_args:
                                                        shortcut_data_dir = re.search(r'--user-data-dir="?([^"]+)"?',
                                                                                      shortcut_args)
                                                        if shortcut_data_dir and self.normalize_path(
                                                                shortcut_data_dir.group(1)) == self.normalize_path(
                                                                data_path):
                                                            # 找到匹配的快捷方式，从文件名提取编号
                    ...
                    def update_ui():
                        ...
                        for window in windows:
                            self.window_list.insert("", "end", values=(
                            "", f"{window['number']}", window['title'], "", window['hwnd']))
    ...
    def set_taskbar_icons(self):
        ...
        # 确认操作
        choice = messagebox.askyesnocancel("选择操作",
                                           "选择要执行的操作：\n是 - 设置自定义图标\n否 - 恢复原始设置\n取消 - 不执行任何操作")
        ...
        try:
            ...
            if not is_admin():
                ...
                def show_admin_prompt():
                    result = messagebox.askquestion("权限提示",
    ...
    def show_settings_dialog(self):
        ...
        ttk.Button(
            shortcut_frame,
            text="浏览",
            command=lambda: shortcut_path_var.set(
                filedialog.askdirectory(initialdir=shortcut_path_var.get() or os.getcwd()))
        ).pack(side=tk.LEFT)
        ...
        # clash配置文件目录
        clash_frame = ttk.Frame(settings_frame)
        clash_frame.pack(fill=tk.X, pady=5)
        ttk.Label(clash_frame, text="clash exe文件目录:").pack(side=tk.LEFT)
        clash_dir_var = tk.StringVar(value=self.clash_dir or settings.get('clash_dir', ''))
        clash_dir_entry = ttk.Entry(clash_frame, textvariable=clash_dir_var)
        clash_dir_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        self.setup_right_click_menu(clash_dir_entry)
        ttk.Button(
            clash_frame,
            text="浏览",
            command=lambda: clash_dir_var.set(filedialog.askdirectory(initialdir=clash_dir_var.get() or os.getcwd()))
        ).pack(side=tk.LEFT)
    ...
    def save_settings_dialog(self, dialog, shortcut_path, cache_dir, icon_dir, screen, clash_dir):
        """保存设置对话框中的设置"""
        try:
                            ...
                            if is_floating_layer:
                                ...
                                for sync_popup in sync_popups:
                                    ...
                                    size_match = 1.0 - min(1.0, (abs(sync_width - popup_width) / max(popup_width, 1) +
                                                                 abs(sync_height - popup_height) / max(popup_height,
                                                                                                       1)) / 2)
                                    ...
                                    pos_match = 1.0 - min(1.0, (abs(sync_rel_x - rel_x) + abs(sync_rel_y - rel_y)) /
                                                          max(sync_rect[2] - sync_rect[0] + sync_rect[3] - sync_rect[1],
                                                              1))
                                ...
                                for sync_popup in sync_popups:
                                    ...
                                    sync_width = sync_rect[2] - sync_rect[0]
                                    sync_height = sync_rect[3] - sync_rect[1]
                                    size_match = min(1.0, 1.0 - (
                                                abs(sync_width - popup_width) + abs(sync_height - popup_height)) /
            ...
            try:
                ...
                if wParam == win32con.WM_MOUSEWHEEL and self.is_sync:
                    ...
                    if not is_master_window and not is_master_plugin:
                        return ctypes.windll.user32.CallNextHookEx(self.wheel_hook_id, nCode, wParam,
                    ...
                    if is_chrome_window and not is_uncontrollable_window:
                        ...
                        print(
                            f"拦截滚轮事件: 窗口={current_window}, 类型={'主窗口' if is_master_window else '主窗口插件' if is_master_plugin else '其他'}, wheel_delta={wheel_delta}")
    ...
    def input_random_number(self):
        ...
        try:
            ...
            print(
                f"准备为{len(selected_windows)}个窗口生成随机数 (范围: {min_val}-{max_val}, 覆盖: {overwrite}, 延迟: {delayed})")
